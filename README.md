...
| `experiments/rust/ai_learning.rs` | Rust AI-cell learning experiment: local Q-learning over decentralized cells |
| `experiments/rust/chaos_monkey.rs` | Rust resilience experiment: random cell failures and continued routing |
| `experiments/rust/scaling.rs` | Rust scaling experiment: routing performance as the cell grid grows |
| `experiments/rust/targeted_failure.rs` | Rust targeted-failure experiment: concentrated cell failures and rerouting |
| `experiments/rust/online_adaptation.rs` | Rust Experiment 5: online adaptation to previously unseen failure conditions |
...
The Experiment 4 Rust suite is split across four runnable binaries:

- `experiments/rust/ai_learning.rs` — local Q-learning experiment for decentralized cells.
- `experiments/rust/chaos_monkey.rs` — random-failure resilience.
- `experiments/rust/scaling.rs` — scaling behavior as the grid grows.
- `experiments/rust/targeted_failure.rs` — concentrated/targeted failure and rerouting.
...
`experiments/rust/online_adaptation.rs` tests a more specific hypothesis: whether a learned decentralized policy can adapt after the environment changes, rather than merely replaying a policy learned before the failure.
