...
| Path | What it is |
|---|---|
| [`cellos/__init__.py`](./cellos/__init__.py) | Core reference implementation: `Cell`, `Fabric` (decentralized mesh), `CentralBus` (shared-bus baseline), plus the `fabric_batch_completion` / `bus_batch_completion` contention model used by the concurrency benchmark |
...
| [`ai_learning.rs`](./ai_learning.rs) | Rust AI-cell learning experiment: local Q-learning over decentralized cells |
| [`chaos_monkey.rs`](./chaos_monkey.rs) | Rust resilience experiment: random cell failures and continued routing |
| [`scaling.rs`](./scaling.rs) | Rust scaling experiment: routing performance as the cell grid grows |
| [`targeted_failure.rs`](./targeted_failure.rs) | Rust targeted-failure experiment: concentrated cell failures and rerouting |
| [`online_adaptation.rs`](./online_adaptation.rs) | Rust Experiment 5: online adaptation to previously unseen failure conditions |

**All Python experiments are directly browsable and runnable from the repo root — no download or extraction required.** The Rust experiments depend on the `cellos_rs` crate (see *Running the Rust experiments*). The ZIP remains available as a complete, self-contained snapshot of the v0.1 package.
...
A structural discrete-event simulation (`fabric_batch_completion` / `bus_batch_completion` in `cellos/__init__.py`) — not a multi-threaded or multi-process hardware benchmark, since Python's GIL makes that comparison meaningless for this workload. It loads the mesh and bus with an increasing number of *simultaneous* events on a 16×16 grid and counts simulated ticks to full completion (50 trials/level).
...
## Rust experiments — AI-cell learning and adaptation

The repository also includes a Rust experimental track that complements the Python/MARL results above. These are software-only experiments and should be read as additional evidence about decentralized cell behavior, resilience, learning, and adaptation.

### Experiment 4 — Rust decentralized AI-cell learning

The Experiment 4 Rust suite is split across four binaries:

- `ai_learning.rs` — local Q-learning experiment for decentralized cells.
- `chaos_monkey.rs` — random-failure resilience.
- `scaling.rs` — scaling behavior as the grid grows.
- `targeted_failure.rs` — concentrated/targeted failure and rerouting.

Together, these tests extend the software evidence from fault containment and contention into a Rust implementation with local learning and cell-level failure scenarios.

### Experiment 5 — Rust online adaptation under unseen failures

`online_adaptation.rs` tests a more specific hypothesis: whether a learned decentralized policy can adapt after the environment changes, rather than merely replaying a policy learned before the failure.

It compares:

1. a BFS oracle baseline,
2. a learned policy frozen after training, and
3. the same learned policy with online updates enabled during the test.

The experiment uses multiple fixed seeds and separates the training failure distribution from the evaluation failure distributions. The evaluation includes an unseen central failure cluster and unseen random failures.

This experiment is intended as evidence about **adaptive decentralized behavior**, not as proof of general intelligence.

### Running the Rust experiments

> **Note:** these files import the `cellos_rs` library crate (`Fabric`, `CentralHub`, `Xorshift`, …), which is not yet published in this repository. Until it is added, the Rust sources are provided for reading; they cannot be built from this repo alone.

From the Rust Cargo project (with `cellos_rs` available):

```bash
cargo run --release --bin ai_learning
cargo run --release --bin chaos_monkey
cargo run --release --bin scaling
cargo run --release --bin targeted_failure
cargo run --release --bin online_adaptation
```

Experiment 5 also writes `experiment5_results.json` so the aggregate results can be preserved and inspected separately from console output.
...
python3 -c "from cellos import Fabric; f = Fabric(dim=(8,8)); print(f.pulse())"
...
- The current evidence remains **toy-scale**: the Python track uses grids up to 20×20 and up to 48 task instances; the Rust track adds cell-failure, scaling, local Q-learning, and online-adaptation simulations. These are tabular/software experiments, not production-scale AI systems.
